...
*/
const numCashSales = transactions.filter(transaction => transaction.type === 'sale' && transaction.paymentMethod === 'cash').length;
...
*/
const allVendors = transactions.filter(transaction => transaction.vendor).map(transaction => transaction.vendor);
...
*/
const uniqueCustomers = new Set(transactions.filter(transaction => transaction.customer).map(transaction => transaction.customer));
...
*/
const bigSpenders = transactions.filter(transaction => transaction.type === 'sale' && transaction.items.length > 4).map(transaction => { return {name: transaction.customer, numItems: transaction.items.length}});
...
*/
const sumOfSmallestPurchase = transactions.filter(transaction => transaction.type == 'purchase').map(transaction => transaction.items.map(item => item.price).reduce((a, b) => a + b, 0)).sort()[0];
